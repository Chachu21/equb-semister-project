...
const Form = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      // Convert image files to base64 before sending them to the backend
      const base64Front = await convertToBase64(data.front[0]);
      const base64Back = await convertToBase64(data.back[0]);
  ...
  let bodyContent = (
    <div className="flex flex-col gap-4">
      ...
      <Input
        ...
        label="ID Back"
        register={register}
        placeholder="back part of your ID"
        required
        onChange={(e) => {
